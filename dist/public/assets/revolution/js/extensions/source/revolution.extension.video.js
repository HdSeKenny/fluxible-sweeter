...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		resetVideo: function (_nc, opt) {
			switch (_nc.data('videotype')) {
				case "youtube":
					...
					if (_nc.find('.tp-videoposter').length == 0) punchgs.TweenLite.to(_nc.find('iframe'), 0.3, { autoAlpha: 1, display: "block", ease: punchgs.Power3.easeInOut });
					break;
		...
		prepareCoveredVideo: function (asprat, opt, nextcaption) {
			...
			if (od > vd) punchgs.TweenLite.to(ifr, 0.001, { height: nvh + "%", width: "100%", top: -(nvh - 100) / 2 + "%", left: "0px", position: "absolute" });else punchgs.TweenLite.to(ifr, 0.001, { width: nvw + "%", height: "100%", left: -(nvw - 100) / 2 + "%", top: "0px", position: "absolute" });
		...
		checkVideoApis: function (_nc, opt, addedApis) {
			...
			if ((_nc.data('vimeoid') != undefined || _nc.find('iframe').length > 0 && _nc.find('iframe').attr('src').toLowerCase().indexOf('vimeo') > 0) && addedApis.addedvim == 0) {
				...
				jQuery('head').find('*').each(function () {
					if (jQuery(this).attr('src') == httpprefix + "://f.vimeocdn.com/js/froogaloop2.min.js") loadit = false;
				});
	...
	var htmlvideoevents = function (_nc, opt, startnow) {
		...
		if (_nc.data('bgvideo') == 1 && (_nc.data('videoloop') === "none" || _nc.data('videoloop') === false)) pforv = false;
