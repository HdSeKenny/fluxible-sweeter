  ...
  ***************************************/
		animateSingleCaption: function (_nc, opt, offsetx, offsety, i, recalled, triggerforce) {
   		...
   		_nc.removeClass("tp-videolayer");							
			...
			if (!triggerforce) if ($lts == "reset" && $start != "bytrigger") {
			...
			if ($progress < 1 && _nc.data('outstarted') != 1 || staticdirection == 2 || triggerforce) {
						...
						_nc.hover(function (e) {
							...
							if (intl && intl.progress() == 1) {
								...
								if (nc.data('newhoveranim') === undefined || nc.data('newhoveranim') === "none") {
									...
									if ($svg.svg) nc.data('newsvghoveranim', punchgs.TweenLite.to($svg.svg, t.speed, nc.data('hoversvg').anim));
								} else {
				...
				if (staticdirection != 2 || triggerforce) {
					...
					newtl.eventCallback("onStart", function () {
						...
						if (_nc.data('iframes')) _nc.find('iframe').each(function () {
				...
				if (($progress === 0 || staticdirection == 2) && $end !== "bytrigger" && !triggerforce && $end != "sliderleave") {
					if ($end != undefined && (staticdirection == -1 || staticdirection == 2) && $end !== "bytriger") var tot = setTimeout(function () {
     	...
     	var tot = setTimeout(function() {
     		_R.endMoveCaption(_nc,_mw,_pw,opt);
				...
				if (($start != "sliderenter" || $start == "sliderenter" && opt.overcontainer) && (staticdirection == -1 || staticdirection == 1 || triggerforce || staticdirection == 0 && $progress < 1 && _nc.hasClass("rev-static-visbile"))) if ($progress < 1 && $progress > 0 || $progress == 0 && $start != "bytrigger" && $lts != "keep" || $progress == 0 && $start != "bytrigger" && $lts == "keep" && $cts == "on" || $start == "bytrigger" && $lts == "keep" && $cts == "on") {
					tl.resume($time);
		...
		endMoveCaption: function (_nc, _mw, _pw, opt) {
			...
			if (_nc.data('timeline')) _nc.data('timeline').pause();else if (_nc.data('_pw') === undefined) return;
		...
		removeTheCaptions: function (actli, opt) {
			...
			if (opt.endtimeouts && opt.endtimeouts.length > 0) jQuery.each(opt.endtimeouts, function (i, timeo) {
				clearTimeout(timeo);
			});
		...
		if (_nc.hasClass("tp-static-layer")) {
			...
			if (dir === "in") {
				...
				if (!_nc.hasClass("rev-static-visbile")) {
					...
					if (e == ai || s > ai || e < ai) a = 2;else a = 0;
				}
			} else {
				// IF STATIC ITEM CURRENTLY NOT VISIBLE
				if (_nc.hasClass("rev-static-visbile")) {
		...
		if (!nc.hasClass("tp-splitted")) {
			...
			if (level != "rekursive") {
				...
				if (nc.data('svg_src') != undefined) {
					if (obj.color != "nopredefinedcolor") punchgs.TweenLite.set(nc.find('svg'), { fill: obj.color, overwrite: "auto" });else punchgs.TweenLite.set(nc.find('svg'), { fill: obj.styleProps.color, overwrite: "auto" });
				}
			}
	...
	var callCaptionLoops = function (el, factor) {
		...
		if (el.hasClass("rs-wave")) {
			if (el.data('loop-timeline') == undefined) {
				...
				el.data('loop-timeline').append(new punchgs.TweenLite.fromTo(angobj, speed, { a: 360 }, { a: 0,
					force3D: "auto",
