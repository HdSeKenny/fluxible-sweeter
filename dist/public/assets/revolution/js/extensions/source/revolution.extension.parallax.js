	...
	jQuery.extend(true, _R, {
      ...
      if (opt.parallax && (opt.parallax.ddd_path=="static" || opt.parallax.ddd_path=="both")) {
		...
		checkForParallax: function (container, opt) {
			...
			for (var i = 1; i <= _.levels.length; i++) opt.c.find('.rs-parallaxlevel-' + i).each(function () {
				var pw = jQuery(this),
     	...
     	diffv = _.ddd_lastv || 0;							
					...
					if (_.type == "3D" || _.type == "3d") {
						...
						opt.c.find(sctor).each(function () {
							...
							} else punchgs.TweenLite.to(t, 0.5, { force3D: "auto", rotationY: 0, z: 0, x: 0, y: 0, rotationX: 0, ["z"]: 0, ease: punchgs.Power3.easeOut, overwrite: "all" });
				...
				if (_ISM) window.ondeviceorientation = function (event) {
					...
					if (_.type == "3D" || _.type == "3d") {
						...
						opt.c.find(sctor).each(function () {
							var t = jQuery(this),
							    pl = _.levels[_.levels.length - 1] / 200;
							offsh = diffh * pl, offsv = diffv * pl * 3, offrv = opt.conw == 0 ? 0 : Math.round(diffh / opt.conw * pl * 500) || 0, offrh = opt.conh == 0 ? 0 : Math.round(diffv / opt.conh * pl * 700) || 0, li = t.closest('li'), zz = 0, itslayer = false;
		...
		scrollHandling: function (opt, fromMouse) {
			...
			if (_.type != "3d" && _.type != "3D") {
				if (_.type == "scroll" || _.type == "scroll+mouse" || _.type == "mouse+scroll") opt.c.find(".tp-parallax-container").each(function (i) {
					var pc = jQuery(this),
				...
				opt.c.find('.tp-revslider-slidesli .slotholder, .tp-revslider-slidesli .rs-background-video-layer').each(function () {
					...
					if (l !== undefined || l !== "off") {
						...
						if (jQuery.isNumeric(offsv)) pt.add(punchgs.TweenLite.set(t, { position: "absolute", top: "0px", left: "0px", backfaceVisibility: "hidden", force3D: "true", y: offsv + "px" }), 0);
					}
				});
			}
